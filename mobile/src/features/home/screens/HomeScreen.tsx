...
import { Text, Avatar, Icon, Badge } from '@/components/atoms';
...
type HomeScreenNavigationProp = DrawerNavigationProp<DrawerParamList> & NativeStackNavigationProp<RootStackParamList>;
...
export const HomeScreen = () => {
  ...
  // Queries
  const { data: featuredEvents, refetch: refetchFeatured, isLoading: loadingFeatured } = useFeaturedEvents();
  const { data: upcomingEvents, refetch: refetchUpcoming, isLoading: loadingUpcoming } = useUpcomingEvents();
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      <View className="px-6 py-4 flex-row items-center justify-between bg-white/80 backdrop-blur-md z-10">
        <View className="flex-row items-center">
          ...
          <View>
            <Text variant="caption" color="secondary" className="uppercase tracking-widest text-[10px]">
              Bienvenido a
            </Text>
        ...
        <View className="flex-row items-center space-x-3">
          <View className="flex-row items-center bg-blue-50 px-2 py-1 rounded-full">
            ...
            <Text variant="caption" color="primary" weight="bold">24°</Text>
          </View>
      ...
      >
        ...
        <View className="px-6 mb-8">
          <Text variant="title" weight="bold" className="mb-4">Explorar</Text>
          <View className="flex-row flex-wrap justify-between">
            {CATEGORIES.map((cat) => (
              ...
              >
                <View className={`w-12 h-12 rounded-full items-center justify-center mb-2 ${cat.color.split(' ')[0]}`}>
                  <Text className="text-2xl">{cat.icon}</Text>
                </View>
                <Text variant="body" weight="medium">{cat.name}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <View className="mb-8">
          <View className="flex-row items-center justify-between px-6 mb-4">
            <Text variant="title" weight="bold">Destacados</Text>
            <TouchableOpacity>
              <Text variant="body" color="secondary" weight="medium">Ver todos</Text>
            </TouchableOpacity>
          </View>
          
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="px-6 gap-4">
            {loadingFeatured ? (
               // Skeleton loading would go here
               <Text>Cargando...</Text>
            ) : featuredEvents?.map((event) => (
              <TouchableOpacity key={event.id} className="w-72" onPress={() => handleEventPress(event.id)}>
                <EventCard 
                  title={event.title}
        ...
        {/* Upcoming Events */}
        <View className="px-6">
          <Text variant="title" weight="bold" className="mb-4">Próximamente</Text>
          {loadingUpcoming ? (
            <Text>Cargando...</Text>
