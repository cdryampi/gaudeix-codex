...
export const CustomDrawerContent = (props: DrawerContentComponentProps) => {
  ...
  const MenuItem = ({ icon, label, onPress }: { icon: string; label: string; onPress: () => void }) => (
    ...
    >
      ...
      <Text variant="body" weight="medium" color="primary">{label}</Text>
    </TouchableOpacity>
  );
    ...
    <SafeAreaView className="flex-1 bg-white" edges={['top', 'bottom']}>
      ...
      <View className="px-4 py-4 flex-row items-center justify-between border-b border-gray-100">
        <View className="flex-row items-center">
          ...
          <Text variant="title" weight="bold">Gaudeix</Text>
        </View>
      ...
      <View className="border-t border-gray-100 p-4 bg-gray-50">
        ...
        <Button variant="outline" size="sm" onPress={handleLogout} className="flex-row justify-center items-center">
          <Text variant="body" weight="medium">Cerrar Sesión</Text>
        </Button>
      </View>
    </SafeAreaView>
  );
