...
## 🐍 MANDATORY RUNTUIME: BACKEND VENV

Before instructing any agent to run backend/python commands, you **MUST** ensure the virtual environment is active:

- **Windows**: `.\.venv\Scripts\activate` (from project root, then `cd backend`)
- **Linux/Mac**: `source .venv/bin/activate` (from project root, then `cd backend`)
