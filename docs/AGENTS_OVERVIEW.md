...
- Los subagentes son especializaciones de Jules orientadas a cubrir distintas etapas del ciclo de desarrollo.
- Esta vista permite entender rápidamente el rol de cada subagente y localizar su definición detallada en `/agents`.
- El sistema replica el modelo operativo del repo de referencia (`JulesTest`), ajustado al stack actual: backend Django REST + PostgreSQL, frontend SPA en React/Vite, backoffice en React Admin y un `docker-compose` unificado con subdominios para backend, frontend y backoffice desplegado en Dokploy.
- Cada subagente tiene un rol, un prompt de sistema, herramientas autorizadas y un workflow definido.
...
## Plantilla de prompts (heredada de JulesTest y adaptada)
