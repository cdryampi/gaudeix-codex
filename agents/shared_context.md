# Contexto Compartido - Subagentes Gaudeix Codex

> 📌 **Propósito**: Este documento centraliza el estado actual del proyecto y los estándares técnicos que todos los subagentes deben consultar antes de ejecutar cualquier tarea.
...
- **Versión actual**: 0.1.0 (en desarrollo inicial)
- **Fase**: Reconstrucción desde cero del monolito legacy

### Repositorio

- **Nombre**: gaudeix-codex
- **Owner**: cdryampi
- **Estructura principal**:
  - `/backend` - API REST con Django + DRF
  - `/frontend` - SPA con React + Vite
  - `/backoffice` - Panel admin con React Admin
  - `/docs` - Documentación técnica (fuente de verdad)
  - `/agents` - Definiciones de subagentes
  - `/chatGPT` - Guías del Director Técnico (exclusivo ChatGPT)
...
- **Linting**: ruff / black
- **Coverage**: pytest-cov (objetivo: >80%)
- **Migraciones**: Django migrations
- **Media**: Almacenamiento persistente (volúmenes Docker)

### Frontend (SPA Pública)
...
- **HTTP Client**: Axios
- **UI**: A definir (Material-UI/Tailwind)
- **i18n**: react-i18next
...
- **Autenticación**: JWT contra misma API
...
- **Desarrollo**: docker-compose.yml unificado
- **Producción**: Dokploy (VPS)
- **CI/CD**: GitHub Actions
- **Subdominios**:
  - Backend API: `api.gaudeix.example.com`
  - Frontend: `www.gaudeix.example.com`
  - Backoffice: `admin.gaudeix.example.com`
- **CORS**: Configurado para subdominios permitidos
- **SSL**: Certificados gestionados por Dokploy

## Estándares de Desarrollo

### Principios Arquitectónicos

1. **Desacoplamiento total**: Backend y frontend se comunican SOLO vía REST API
2. **Versionado de API**: `/api/v1/` para todos los endpoints
3. **Stateless**: JWT en cada request, sin sesiones en servidor
4. **Documentación automática**: OpenAPI/Swagger para todos los endpoints
5. **Testing obligatorio**: Cobertura mínima 80% en backend, 70% en frontend
...
backend/
  {module_name}/
    ...
    tests/
      ...
      test_views.py
      test_serializers.py
    admin.py         # Admin Django (opcional)
...
**Responsable**: Director Técnico (ChatGPT) con apoyo del Integrador
