...
export function getWhenLabel(date, now = new Date()) {
    if (isToday(date, now))
        return "Hoy";
...
export function formatDay(date, locale = DEFAULT_LOCALE) {
    return new Intl.DateTimeFormat(locale, { day: "2-digit" }).format(date);
}
export function formatMonthShort(date, locale = DEFAULT_LOCALE) {
