...
import { isPlaceVisibleInExplorer } from "../utils";
  ...
  return (
    <main className="min-h-screen bg-background-light page-shell-offset">
      <PageHero
        ...
        tone="immersive"
        breadcrumbs={[{ label: "Inicio", href: "/" }, { label: "Lugares" }]}
        metrics={[
                ...
                <div className="flex rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60 p-1">
                  <button
                    ...
                    className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${
                      viewMode === "list"
                        ? "bg-slate-100/80 text-primary shadow-sm ring-1 ring-slate-200"
                  ...
                  <button
                    ...
                    className={`hidden rounded-xl px-4 py-2 text-sm font-semibold transition lg:block ${
                      viewMode === "split"
                        ? "bg-slate-100/80 text-primary shadow-sm ring-1 ring-slate-200"
                        : "text-slate-500 hover:bg-slate-50"
                    }`}
            ...
            {isLoading ? (
              <div className="grid gap-8">
                {[1, 2, 3].map((item) => (
                  <div
                    key={item}
                    className="card-surface h-[320px] animate-pulse bg-slate-100"
                  />
                ))}
              </div>
            ) : places.length ? (
