...
import { ChevronRight, Navigation, Mountain, Timer } from "lucide-react";
                  ...
                  {itineraries.map((itinerary, idx) => {
                    ...
                    if (itinerary.route.track_geojson) {
                      ...
                      if (geo.type === "LineString" && geo.coordinates) {
                        ...
                        paths = geo.coordinates.map((seg: number[][]) =>
                          seg.map((c: number[]) => ({ lat: c[1], lng: c[0] })),
                        );
                      }
                    }
                  ...
                  {hoveredCheckpointId &&
                    (() => {
                      ...
                      for (const it of itineraries) {
                        const found = it.checkpoints.find(
                          (c: any) => c.id === hoveredCheckpointId,
                        );
                        if (found) {
                          hoveredCp = found;
                          break;
              ...
              <div className="absolute inset-0 bg-slate-900/10 backdrop-blur-sm flex items-center justify-center p-6 text-center">
                <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm">
                  ...
                  <p className="text-sm text-slate-500">
                    Elige una ruta en el panel lateral para visualizar su
                    itinerario y puntos de paso en el mapa.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                ...
                <button
                  onClick={() => {
                    ...
                    alert(
                      "¡Enlace copiado al portapapeles! Puedes compartir tu Roadmap.",
                    );
                  }}
