    ...
    const handleTraceRoute = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                setTracingMode(window.google?.maps?.TravelMode?.DRIVING || "DRIVING" as any);
    ...
    return (
        <div className="relative w-full h-full flex flex-col">
            ...
            <button
                ...
                className={`absolute top-4 right-4 z-10 shadow-lg px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-colors flex items-center gap-2 ${tracingMode
            ...
            >
                ...
                {parsedTrackPaths
                    ? parsedTrackPaths.map((path: google.maps.LatLngLiteral[], index: number) => (
                        <PolylineF
                            key={`track-${index}`}
                            path={path}
                            options={{
