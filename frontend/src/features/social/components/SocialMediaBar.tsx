...
import { Instagram, Facebook, Ticket, Image, Link as LinkIcon, Twitter, Youtube } from "lucide-react";
...
export function SocialMediaBar({ scrolled }: { scrolled: boolean }) {
    ...
    useEffect(() => {
        listSocialLinks().then((data) => {
            // Filter out 'ticket' related links (Codetickets) as they have their own CTA now
            const filtered = data.filter(link => !link.icon_class.includes("ticket"));
            setLinks(filtered);
        });
    }, []);
            ...
            {links.map((link) => {
                ...
                return (
                    <a
                        ...
                        className={`p-2 rounded-full transition-all backdrop-blur-sm group ${scrolled
