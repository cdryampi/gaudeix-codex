...
export const HomeSubHero = () => {
  ...
  const items = [
    ...
    },
    {
      ...
      bg: "bg-secondary/5 ring-secondary/10",
        ...
        <div className="mx-auto max-w-5xl overflow-hidden rounded-[2.5rem] bg-white p-2 shadow-[0_32px_80px_rgba(0,0,0,0.1)] ring-1 ring-slate-200/50 md:rounded-[3.5rem] md:p-4">
          <div className="grid grid-cols-2 lg:grid-cols-4">
            {items.map((item) => (
              ...
              >
                <div
                  className={`relative flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ring-1 transition-all duration-500 group-hover:-translate-y-1 group-hover:scale-110 md:h-16 md:w-16 md:rounded-[1.25rem] ${item.bg} ${item.glow} group-hover:shadow-2xl`}
                >
                  <item.icon
                    className={`h-7 w-7 transition-transform duration-500 group-hover:rotate-6 md:h-8 md:w-8 ${item.color}`}
                  />
                  <div
                    className="absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-20"
                    style={{
                      background:
