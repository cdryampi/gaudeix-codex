...
export const siteSettingsApi = {
    async get() {
        const response = await apiClient.get(API_ENDPOINTS.STATIC_PAGES.LIST.replace("static-pages", "site-settings"));
        return normalize(response.data);
    },
    async update(payload) {
        const response = await apiClient.patch(API_ENDPOINTS.STATIC_PAGES.DETAIL(String(1)).replace("static-pages", "site-settings"), payload);
        return normalize(response.data);
    },
};
