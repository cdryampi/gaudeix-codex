...
vi.mock("@/components/common", () => ({
    ...
    PageHeader: ({ title, actions, }) => (_jsxs("div", { children: [_jsx("h1", { children: title }), actions] })),
}));
const MOCK_LINKS = [
    {
        id: 1,
...
describe("SocialLinksPage CRUD", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        socialLinksApi.getAll.mockResolvedValue(MOCK_LINKS);
    });
    it("renders social links and fetches data", async () => {
        render(_jsx(SocialLinksPage, {}));
    ...
    it("deletes a social link", async () => {
        socialLinksApi.delete.mockResolvedValue({});
        render(_jsx(SocialLinksPage, {}));
        await waitFor(() => {
            expect(screen.getByText("Facebook")).toBeInTheDocument();
        });
