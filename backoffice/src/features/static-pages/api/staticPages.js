...
export const staticPagesApi = {
  async list(params) {
    const response = await apiClient.get(API_ENDPOINTS.STATIC_PAGES.LIST, {
      params,
    });
  ...
  async create(payload) {
    const response = await apiClient.post(
      API_ENDPOINTS.STATIC_PAGES.LIST,
      payload,
    );
  ...
  async update(id, payload) {
    const response = await apiClient.patch(
      API_ENDPOINTS.STATIC_PAGES.DETAIL(String(id)),
      payload,
    );
    return normalizeStaticPage(response.data);
  },
  ...
  async autoTranslate(id, data) {
    const response = await apiClient.post(
      API_ENDPOINTS.STATIC_PAGES.AUTO_TRANSLATE(String(id)),
      data,
    );
