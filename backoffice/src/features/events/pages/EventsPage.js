  ...
  const filtered = useMemo(() => {
    return events.filter((event) => {
      ...
      const tagsText = (event.tags || [])
        ...
        .join(" ");
      const text =
        `${event.title} ${event.summary ?? ""} ${event.description ?? ""} ${event.venue_name ?? ""} ${event.location_text ?? ""} ${event.price_text ?? ""} ${event.category_name ?? ""} ${event.category_slug ?? ""} ${tagsText}`.toLowerCase();
    ...
    children: [
      _jsx(PageHeader, {
        ...
        actions: _jsxs(Button, {
          ...
          size: "sm",
          children: [_jsx(Plus, { className: "mr-2 h-4 w-4" }), "Nuevo evento"],
        }),
      }),
      _jsx(EventsFilters, {
        search: search,
