...
export function PlacesPage() {
  ...
  const [editing, setEditing] = useState<Place | undefined>();
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Places"
        description="Gestiona los lugares y sus traducciones"
        actions={
      ...
      <Card className="border-border bg-card">
        <CardContent className="p-0">
          {loading ? (
            <div className="flex h-48 items-center justify-center text-muted-foreground">Cargando lugares...</div>
          ) : error ? (
            <div className="flex h-48 items-center justify-center text-destructive">{error}</div>
          ) : (
            <PlacesTable places={paginated} onEdit={handleEdit} onDelete={handleDelete} />
          )}
        </CardContent>
      </Card>
