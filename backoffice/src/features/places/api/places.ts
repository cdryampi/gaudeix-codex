...
export const placesApi = {
  async getAll(params?: { search?: string; is_published?: string; category?: string | number }) {
    const response = await apiClient.get<Place[]>(API_ENDPOINTS.PLACES.LIST, { params });
    return response.data.map(normalizePlace);
  },

  async create(payload: PlacePayload) {
    const response = await apiClient.post<Place>(API_ENDPOINTS.PLACES.LIST, payload);
    return normalizePlace(response.data);
  },

  async update(id: number, payload: PlaceUpdatePayload) {
    const response = await apiClient.patch<Place>(API_ENDPOINTS.PLACES.DETAIL(String(id)), payload);
    return normalizePlace(response.data);
  },
